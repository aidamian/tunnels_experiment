...
def parse_args() -> argparse.Namespace:
  ...
  ``python3 clients/src/bridge/start_local_bridges.py``

  Start only PostgreSQL and verify it:

  ``python3 clients/src/bridge/start_local_bridges.py --service postgres --verify``
  """
  ...
  parser.add_argument(
    "--service",
    choices=["all", "postgres", "neo4j", "neo4j_bolt"],
    default="all",
  ...
  parser.add_argument(
    "--postgres-port",
    type=int,
    help="Override the PostgreSQL localhost port from clients/services.json.",
  )
  parser.add_argument(
    "--neo4j-port",
    type=int,
    help="Override the Neo4j Bolt localhost port from clients/services.json.",
  )
  parser.add_argument(
    "--verify",
    action="store_true",
...
def selected_specs(args: argparse.Namespace) -> list[Any]:
  ...
  if args.service == "all":
    return specs
  selected_key = SERVICE_SELECTOR_MAP[args.service]
  return [spec for spec in specs if spec.service_key == selected_key]
...
def main() -> int:
  ...
  >>> # clients/.venv/bin/python clients/src/bridge/start_local_bridges.py

  Start only PostgreSQL and verify it:

  >>> # clients/.venv/bin/python clients/src/bridge/start_local_bridges.py --service postgres --verify
  """
